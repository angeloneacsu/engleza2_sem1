...
gameType_gradAndDrop.prototype.init = function() {  activ_help_btn = true;$('.wrapper').addClass('scale_reset');

    this.is_verification_started = false;
    this.sstare_iteratie = 'corect';
    this.local_arr_validitate = [];
    this.local_arr_valid_obj = []; $('.avatar').show(); $('.avatar_corect').hide();  $('.avatar_gresit').hide();
...
var _this = this;this.first_init = false;$('#' + this.html_hook_id + ' .reia_sarcina').on("vclick", function() {_this.init();});
...
gameType_gradAndDrop.prototype.verifica = function() {
    ...
    if ($.inArray("no_choice", local_arr_validitate) > -1) {
      ...
      var corect=local_arr_validitate.length; for(var i=0;i<local_arr_validitate.length;i++){ if(local_arr_validitate[i]==false||local_arr_validitate[i]=='no_choice_false'){ corect--  } } var rezultat=corect+'/'+local_arr_validitate.length; insertResponse(this.data_obj.nr_pagina, this.data_obj.nr_exercitiu, rezultat , this.stare_iteratie); this.take_action();
...
play_audio_valid('corect_' + random_sound_nr);
         ...
         show_anim(_this.stare_iteratie); if (_this.stare_iteratie == 'wrong') { $(player_valid).on('ended', function() {
